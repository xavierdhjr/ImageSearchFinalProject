...
public:
    ...
    */
	...
	{	
		...
		printf("computed node statistics\n");
		computeClustering(root, indices, size_, branching,0);
		printf("computed clustering\n");
	}
    ...
    {
		printf("CALCULATIN DA CLUSTER CENTERS!\n");
        if (numClusters<1) {
			printf("oh no.\n");
...
//         logger.info("Clusters requested: %d, returning %d\n",numClusters, clusterCount);
		...
		printf("did some forloop magic.\n");
		delete[] clusters;
        printf("deleted the clusters.\n");
        return clusterCount;
    }
...
private:
    ...
    */
	...
	void computeNodeStatistics(KMeansNode node, int* indices, int indices_length) {
		...
		//printf("computeNodeStatistics - init'd vars. size_: %d, indices: %d, indices_length: %d\n",size_, *indices, indices_length);
        memset(mean,0,veclen_*sizeof(float));
		//printf("computeNodeStatistics - did memset\n");
		for (int i=0;i<size_;++i) {
		...
		//printf("computeNodeStatistics - forloop 1\n");
		for (int j=0;j<veclen_;++j) {
			mean[j] /= size_;
		}
		//printf("computeNodeStatistics - forloop 2\n");
		variance /= size_;
		...
		printf("computeNodeStatistics - forloop 3\n");
		node->variance = variance;
		node->radius = radius;
		node->pivot = mean;

		printf("computeNodeStatistics - node pointer stuff\n");
	}
